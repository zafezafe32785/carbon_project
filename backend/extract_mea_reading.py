...
def extract_from_pdf(pdf_path, coords, dpi=300):
    ...
    print(f"[MEA Extract] Rendered at {dpi} DPI: {img.size[0]}x{img.size[1]} pixels")
...
def visualize_coordinates(pdf_path, coords, dpi=300, output_path=None):
    ...
    print(f"[MEA Visualize] Rendered at {dpi} DPI: {img.size[0]}x{img.size[1]} pixels")
...
if __name__ == "__main__":
    coords = {
        "จำนวนหน่วย": (1020, 720, 1200, 870),     # Example for MEA
        "วันที่จดเลขอ่าน": (400, 720, 600, 870), # Example for MEA
    }
    ...
    img = visualize_coordinates(pdf_path, coords, output_path="mea_visualization.png")
    print("Done! Check mea_visualization.png to see the red boxes.")
